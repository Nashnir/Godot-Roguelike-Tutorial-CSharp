...
namespace SuperRogalik
{
    public partial class DungeonGenerator : Node
    {
        ...
        public static readonly Dictionary<string, EntityDefinition> entityTypes = new(){
            ...
            { LIGHTING_SCROLL, ResourceLoader.Load<EntityDefinition>("res://assets/definitions/entities/items/lightning_scroll_definition.tres") },
            { CONFUSION_SCROLL, ResourceLoader.Load<EntityDefinition>("res://assets/definitions/entities/items/confusion_scroll_definition.tres") },
            { FIREBALL_SCROLL, ResourceLoader.Load<EntityDefinition>("res://assets/definitions/entities/items/fireball_scroll_definition.tres") },
        };
            ...
            for (int i = 0; i < numberOfMonsters; i++)
            {
                ...
                if (newEntityPosition.HasValue)
                {
                    Entity newEntity = rng.Randf() < 0.8
                        ? new Entity(dungeon, newEntityPosition.Value, entityTypes[ORC])
                        : new Entity(dungeon, newEntityPosition.Value, entityTypes[TROLL]);
        ...
        private void PlaceItems(MapData dungeon, Rect2I room)
        {
            ...
            for (int i = 0; i < numberOfItems; i++)
            {
                ...
                if (newEntityPosition != null && newEntityPosition.HasValue)
                {
                    ...
                    if (item_chance < 0.7)
                    {
                        newEntity = new Entity(dungeon, newEntityPosition.Value, entityTypes[HEALTH_POTION]);
                    }
                    else if (item_chance < 0.8)
                    {
                        newEntity = new Entity(dungeon, newEntityPosition.Value, entityTypes[FIREBALL_SCROLL]);
                    }
                    else if (item_chance < 0.9)
                    {
                        newEntity = new Entity(dungeon, newEntityPosition.Value, entityTypes[CONFUSION_SCROLL]);
                    }
                    else
                    {
                        newEntity = new Entity(dungeon, newEntityPosition.Value, entityTypes[LIGHTING_SCROLL]);
                    }
                    dungeon.Entities.Add(newEntity);
                }
            }
