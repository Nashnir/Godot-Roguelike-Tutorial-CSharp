using Godot;
using System.Linq;
using System.Threading.Tasks;

namespace SuperRogalik
{
    public partial class Game : Node2D
    {
        [Signal]
        public delegate void PlayerCreatedEventHandler(Entity player);

        Vector2I playerGridPos = Vector2I.Zero;
        Entity player;
        readonly EntityDefinition playerDefinition = ResourceLoader
            .Load<EntityDefinition>("res://assets/definitions/entities/actors/entity_definition_player.tres");

        InputHandler inputHandler;
        Map map;
        Camera2D camera;

        readonly PackedScene LevelUpMenuScene = ResourceLoader.Load<PackedScene>("res://src/GUI/LevelUpMenu/LevelUpMenu.tscn");

        public override void _Ready()
        {
            inputHandler = GetNode<InputHandler>("InputHandler");
            map = GetNode<Map>("Map");
            camera = GetNode<Camera2D>("Camera2D");
        }

        public void NewGame()
        {
            player = new Entity(null, Vector2I.Zero, Entity.PLAYER);
            AddPlayerStartEquipment(Entity.DAGGER);
            AddPlayerStartEquipment(Entity.LEATHER_ARMOR);
            player.LevelComponent.LevelUpRequired += async () => await OnPlayerLevelUpRequestedAsync();
            EmitSignal(SignalName.PlayerCreated, player);
            RemoveChild(camera);
            player.AddChild(camera);
            map.Generate(player);
            map.UpdateFov(player.GridPosition);
            MessageLog.SendMessage("Hello and welcome, adventurer, to yet another dungeon!", GameColors.WELCOME_TEXT);
            camera.CallDeferred(Camera2D.MethodName.MakeCurrent);
        }

        public bool LoadGame()
        {
            player = new Entity(null, Vector2I.Zero, "");
            RemoveChild(camera);
            player.AddChild(camera);
            if (!map.LoadGame(player))
                return false;
            player.LevelComponent.LevelUpRequired += async () => await OnPlayerLevelUpRequestedAsync();
            EmitSignal(SignalName.PlayerCreated, player);
            map.UpdateFov(player.GridPosition);
            MessageLog.SendMessage("Welcome back, adventurer!", GameColors.WELCOME_TEXT);
            camera.CallDeferred(Camera2D.MethodName.MakeCurrent);
            return true;
        }

        public override async void _PhysicsProcess(double delta)
        {
            var action = await inputHandler.GetActionAsync(player);
	        if (action != null && action.Perform())
            {
                HandleEnemyTurns();
                map.UpdateFov(player.GridPosition);
            }
        }

        private void HandleEnemyTurns()
        {
            foreach (var entity in GetMapData().GetActors().Where(a => a != player && a.IsAlive()))
            {
                entity.AIComponent.Perform();
            }
        }

        public MapData GetMapData() =>
            map.MapData;

        public async Task OnPlayerLevelUpRequestedAsync()
        {
            var levelUpMenu = LevelUpMenuScene.Instantiate<LevelUpMenu>();
            AddChild(levelUpMenu);
            levelUpMenu.Setup(player);
            SetPhysicsProcess(false);
            await ToSignal(levelUpMenu, LevelUpMenu.SignalName.LevelUpCompleted);
            CallDeferred(Node.MethodName.SetPhysicsProcess, true);
        }

        private void AddPlayerStartEquipment(string item_key)
        {
            var item = new Entity(null, Vector2I.Zero, item_key);
            player.InventoryComponent.Items.Add(item);
            player.EquipmentComponent.ToggleEquip(item, false);
        }
    }
}
